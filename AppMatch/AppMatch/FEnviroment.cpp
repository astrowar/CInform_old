...
namespace VM {
    ...
    CRelationDescriptionNode::CRelationDescriptionNode(std::string _named, HGenericKind _vkind) : named(_named),
                                                                                                  vkind(_vkind) {
    }

    CRelationDescriptionNodeGroup::CRelationDescriptionNodeGroup(std::string _named, HGenericKind _vkind)
            : CRelationDescriptionNode(_named, _vkind) {
    }

    CRelationDescriptionNodeMany::CRelationDescriptionNodeMany(std::string _named, HGenericKind _vkind)
            : CRelationDescriptionNode(_named, _vkind) {
    }

    CRelationDescription::CRelationDescription(std::string _named, HRelationDescriptionNode _node1,
                                               HRelationDescriptionNode _node2, bool _symmetric) : node1(_node1),
                                                                                                   node2(_node2),
                                                                                                   named(_named),
                                                                                                   symmetric(
                                                                                                           _symmetric) {
        if (_symmetric) {
            if ((_node1->isMany() != _node2->isMany()) ||
                (isEqual(_node1->vkind.get(), _node2->vkind.get()) != Equals)) {
    ...
    CRelationInstance::CRelationInstance(CRelationDescription *_relDesc, HGenericValue val, HGenericValue val2)
            : relDesc(_relDesc), item1(val), item2(val2) {
    }
