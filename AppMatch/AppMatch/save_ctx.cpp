...
int  alloc_slot(SaveContext *ctx) { return 0; };
...
template<typename T>  void save_vector(std::vector<std::shared_ptr<T> > s, SaveContext *ctx){};
 
template<typename T>  void save_list(std::list<std::shared_ptr<T> > s, SaveContext *ctx) {};
...
void raiseError() {};
  ...
  int  save_CBlockList_base(HBlockList x, SaveContext *ctx);
  ...
  int  save_CBlockInterpreter(Interpreter::HBlockInterpreter x, SaveContext *ctx);
  ...
  int   save_CBlockList(HBlockList x, SaveContext * ctx)
  {
	  ...
	  if (t == BlockType::BlockList) return save_CBlockList_base(std::static_pointer_cast < CBlockList > (x), ctx);
  ...
  int   save_CBlock(HBlock x, SaveContext * ctx)
  {
	  ...
	  if (t == BlockType::BlockNoum) return save_CBlockNoum(std::static_pointer_cast < CBlockNoum > (x), ctx);
	  ...
	  if (t == BlockType::BlockActionCall) return save_CBlockActionCall(std::static_pointer_cast < CBlockActionCall > (x), ctx);
	  ...
	  if (t == BlockType::BlockAction) return save_CBlockAction(std::static_pointer_cast < CBlockAction > (x), ctx);
	  ...
	  if (t == BlockType::BlockInstance) return save_CBlockInstance(std::static_pointer_cast < CBlockInstance > (x), ctx);
  ...
  int   save_CBlockInstance(HBlockInstance x, SaveContext * ctx)
  {
	  const BlockType t = x->type();
	  if (t == BlockType::BlockInstanceNamed) return save_CBlockInstanceNamed(std::static_pointer_cast < CBlockInstanceNamed > (x), ctx);
	  if (t == BlockType::BlockInstance) return save_CBlockInstance_base(std::static_pointer_cast < CBlockInstance > (x), ctx);
  ...
  int   save_CBlockAction(HBlockAction x, SaveContext * ctx)
  {
	  ...
	  if (t == BlockType::BlockActionInstance) return save_CBlockActionInstance(std::static_pointer_cast < CBlockActionInstance > (x), ctx);
	  if (t == BlockType::BlockActionNamed) return save_CBlockActionNamed(std::static_pointer_cast < CBlockActionNamed > (x), ctx);
	  if (t == BlockType::BlockAction) return save_CBlockAction_base(std::static_pointer_cast < CBlockAction > (x), ctx);
  ...
  int   save_CBlockActionCall(HBlockActionCall x, SaveContext * ctx)
  {
	  ...
	  if (t == BlockType::BlockActionCallNamed) return save_CBlockActionCallNamed(std::static_pointer_cast < CBlockActionCallNamed > (x), ctx);
	  if (t == BlockType::BlockStaticDispatch) return save_CBlockStaticDispatch(std::static_pointer_cast < CBlockStaticDispatch > (x), ctx);
	  if (t == BlockType::BlockActionCall) return save_CBlockActionCall_base(std::static_pointer_cast < CBlockActionCall > (x), ctx);
  ...
  int   save_CBlockNoum(HBlockNoum x, SaveContext * ctx)
  {
	  const BlockType t = x->type();
	  if (t == BlockType::BlockNoumSupl) return save_CBlockNoumSupl(std::static_pointer_cast < CBlockNoumSupl > (x), ctx);
	  if (t == BlockType::BlockNoum) return save_CBlockNoum_base(std::static_pointer_cast < CBlockNoum > (x), ctx);
  ...
  int  save_CBlockVerb(HBlockVerb x, SaveContext *ctx)
  {
	  ...
	  save_type(x->type(), ctx);
	  save_string(x->named, ctx);
	  save_list<CBlockVerbConjugation>(x->conjugations, ctx);
	  end_slot(ctx);
	  return  slot;
  ...
  int  save_CBlockMatchAND(HBlockMatchAND x, SaveContext *ctx)
  {
	  ...
	  const int slot = alloc_slot(ctx);
	  save_type(x->type(), ctx);
	  save_list<CBlockMatch>(x->matchList, ctx);
	  end_slot(ctx);
	  return  slot;
  ...
  int  save_CBlockTextSentence(HBlockTextSentence x, SaveContext *ctx)
  {
	  ...
	  const int slot = alloc_slot(ctx);
	  save_type(x->type(), ctx);
	  save_list<CBlock>(x->contents, ctx);
	  end_slot(ctx);
	  return  slot;
  ...
  int  save_CBlockControlSelect(HBlockControlSelect x, SaveContext *ctx)
  {
	  ...
	  save_type(x->type(), ctx);
	  save_id(_block_seletor, ctx);
	  save_list<CBlockControlSelectItem>(x->block_selectList, ctx);
	  save_id(_block_else, ctx);
	  end_slot(ctx);
	  return  slot;
  ...
  int  save_CBlockEnums(HBlockEnums x, SaveContext *ctx)
  {
	  ...
	  const int slot = alloc_slot(ctx);
	  save_type(x->type(), ctx);
	  save_vector<CBlockNoum>(x->values, ctx);
	  end_slot(ctx);
	  return  slot;
